...
async def purchases_entry(message: Message, state: FSMContext):
    ...
    sent = await message.answer(
        ...
        "<i>Если вы видите, что инструмент, материал или расходник закончился и это повлечет остановку производства: создавайте заявку в этом разделе.</i> \n\n"
        ...
        "1. Выберите приоритет заявки; 2. Напишите что нужно закупить и прикрепите фото по необходимости.\n\n"
        "✅ Например: Перчатки размер М 10 упаковок\n\n"
...
async def purchases_receive_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"✅ <b>Закупка #{int(purchase.id)}</b> создана.\n\n"
